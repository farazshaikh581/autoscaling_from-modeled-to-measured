...
PROMETHEUS_URL = "http://localhost:9090"
url_app_service = "http://192.168.122.2:30001/factor"
KUBECTL_CMD = "kubectl"
...
def get_random_days(df, n_days, train_days, test_days, seed=42):
    ...
    if len(all_days) < n_days:
        raise ValueError(f"Not enough unique days. Found {len(all_days)}, need {n_days}.")
    random.seed(seed)
...
class CustomTransformerExtractor(BaseFeaturesExtractor):
    ...
    def forward(self, observations):
        ...
        features = torch.nn.functional.relu(self.output_fc(pooled_output))
        return features
...
def get_all_node_metrics():
    try:
        out = subprocess.check_output(
            f"{KUBECTL_CMD} top nodes --no-headers",
        ...
        for line in out:
            parts = line.split()
            if len(parts) >= 5:
                result[parts[0]] = (parts[2].replace("%",""), parts[4].replace("%",""))
        return result
    except Exception:
        return {}
...
def get_node_names():
    try:
        out = subprocess.check_output(
            f"{KUBECTL_CMD} get nodes --no-headers -o custom-columns=NAME:.metadata.name",
            shell=True, encoding="utf-8", stderr=subprocess.DEVNULL
        ).strip().split(chr(10))
...
class KubernetesEnv(gym.Env):
    def __init__(self, df, minutes, day_list):
        ...
        self.reset()
        self.max_steps = self.minutes_per_day
        self.current_step = 0
        ...
        try:
            ...
            try:
                ...
                if latency_match:
                    ...
                    latency = latency_value / 1000.0 if unit == "ms" else latency_value
            except Exception as e:
    ...
    def step(self, action):
        ...
        if terminated:
            self.current_step = 0
            self.days += 1
        node_metrics = get_all_node_metrics()
        info = {"raw_reward": reward_vector, "p95_latency": p95_latency, "node_metrics": node_metrics}
...
class MultiObjectiveLoggingCallback(BaseCallback):
    ...
    def _on_training_start(self):
        ...
        headers = ["timestep","r_perf","r_cost","r_energy","latency","replicas",
                   "avg_cpu_percent","ram_usage","num_requests","total_cpu_m",
                   "total_ram_mi","success_ratio","pod_power","hpa_target",
                   "masterk8s_cpu", "masterk8s_mem", "worker11_cpu", "worker11_mem", "worker12_cpu", "worker12_mem", "worker13_cpu", "worker13_mem", "worker14_cpu", "worker14_mem"]
    ...
    def _on_step(self) -> bool:
        ...
        if 'raw_reward' in info:
            ...
            nm = info.get("node_metrics", {})   # FIX: define nm here
            ...
            self.csv_writer.writerow({
                ...
                "masterk8s_cpu": nm.get("masterk8s",("0","0"))[0], "masterk8s_mem": nm.get("masterk8s",("0","0"))[1],
                "worker11_cpu": nm.get("worker11",("0","0"))[0], "worker11_mem": nm.get("worker11",("0","0"))[1],
                "worker12_cpu": nm.get("worker12",("0","0"))[0], "worker12_mem": nm.get("worker12",("0","0"))[1],
                "worker13_cpu": nm.get("worker13",("0","0"))[0], "worker13_mem": nm.get("worker13",("0","0"))[1],
                "worker14_cpu": nm.get("worker14",("0","0"))[0], "worker14_mem": nm.get("worker14",("0","0"))[1],
            })
...
def main():
    ...
    parser.add_argument("--profile", choices=["perf_focused","cost_focused","energy_focused","balanced"], default="perf_focused")
    parser.add_argument("--beta2", type=float, default=None)
    parser.add_argument('--num-workers', type=int, default=4)
    parser.add_argument('--worker-cpu', type=str, default='2vCPU')
    parser.add_argument('--worker-ram', type=str, default='16GB')
    parser.add_argument('--cluster', type=str, default='masterk8s')
    parser.add_argument("--run-dir", type=str, default=None)
    args = parser.parse_args()
    WORKER_CONFIG = f"{args.num_workers}x{args.worker_cpu}-{args.worker_ram}"
    ...
    if args.mode == "train":
        ...
        with open(csv_path, "w", newline="") as f:
            ...
            for t in pbar:
                ...
                nm_test = raw_info_step.get("node_metrics", {}) if isinstance(raw_info_step, dict) else {}
                writer.writerow({
                    "timestep": t + 1,
                    "r_perf": r_perf, "r_cost": r_cost, "r_energy": r_energy,
                    ...
                    "masterk8s_cpu": nm_test.get("masterk8s",("0","0"))[0], "masterk8s_mem": nm_test.get("masterk8s",("0","0"))[1],
                    "worker11_cpu": nm_test.get("worker11",("0","0"))[0], "worker11_mem": nm_test.get("worker11",("0","0"))[1],
                    "worker12_cpu": nm_test.get("worker12",("0","0"))[0], "worker12_mem": nm_test.get("worker12",("0","0"))[1],
                    "worker13_cpu": nm_test.get("worker13",("0","0"))[0], "worker13_mem": nm_test.get("worker13",("0","0"))[1],
                    "worker14_cpu": nm_test.get("worker14",("0","0"))[0], "worker14_mem": nm_test.get("worker14",("0","0"))[1],
                })
